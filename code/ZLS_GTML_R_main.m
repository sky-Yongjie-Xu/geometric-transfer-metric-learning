...
for num = 1:10
    ...
    etas = result_zls_hr_sar_gtml_r(num,1);
    etad = result_zls_hr_sar_gtml_r(num,2);
    mium = result_zls_hr_sar_gtml_r(num,3);
    miuc = result_zls_hr_sar_gtml_r(num,4);
    gamma = result_zls_hr_sar_gtml_r(num,5);
    ...
    W1 = KNNGraph_constant(X', options.n_neighbor, flag);
    Lm = diag(sum(W1))-W1;
    ...
    minimize(mium*trace(X'*Lmd*X*M)+miuc*trace(X'*Lcd*X*M)+etas*trace(S*M)+gamma*trace(X'*Lm*X*M))
    subject to
    etad*trace(D*M)==1
    cvx_end
    
    preds = KNN_M(ySr,xSr,M,options.n_neighbor,xTe);
...
acc_gtml_r_mean = mean(acc_test_gtml_r);
acc_gtml_r_std = std(acc_test_gtml_r);
fprintf('%0.4f %0.4f\n',acc_gtml_r_mean,acc_gtml_r_std);
